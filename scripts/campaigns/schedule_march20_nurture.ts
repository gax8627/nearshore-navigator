/**
 * March 20th Nurture Campaign — Warm Lead Follow-Up
 *
...
const SENT_LOG_PATH = path.join(process.cwd(), 'scripts/sent_march20_nurture.json');

const DRY_RUN = process.argv.includes('--dry-run');

// Schedule for TODAY 9:30 AM MT (16:30 UTC)
const SCHEDULED_AT = '2026-03-20T16:30:00.000Z';
...
async function main() {
  ...
  for (const [email, info] of Object.entries(engagement) as [string, any][]) {
    ...
    // Find March 19 or 20 activity
    const marchEvents = history.filter((e: any) =>
      (e.date || '').includes('2026-03-19') || (e.date || '').includes('2026-03-20')
    );
    if (marchEvents.length === 0) continue;

    const hasClick = marchEvents.some((e: any) => ['clicked', 'clicks'].includes(e.type));
    const hasOpen = marchEvents.some((e: any) => ['opened', 'loadedByProxy'].includes(e.type));
  ...
  if (!DRY_RUN) {
    fs.writeFileSync(SENT_LOG_PATH, JSON.stringify(sentEmails, null, 2));
    console.log(`\n✅ Done! ${sentCount} nurture emails queued for Mar 20 @ 9:30AM MT.`);
