/**
 * Immediate 497 Lead Campaign Blast — USMCA Outreach
 *
 * Target: Exactly 497 fresh, unsent contacts from tier lists (Aerospace, Medical, Electronics, Automotive, General)
 * Scheduled: Immediately (5 minutes from run time)
 *
 * Run dry:  npx tsx scripts/campaigns/send_497_blast.ts --dry-run
 * Run live: npx tsx scripts/campaigns/send_497_blast.ts
 */
...
async function main() {
  console.log(`\n🚀 USMCA Immediate 497 Campaign Blast`);
  console.log(`  Scheduled to dispatch dynamically at: ${SCHEDULED_AT}`);
  ...
  for (const [fpath, source] of [[tier1Path, 'tier1'], [tier2Path, 'tier2']]) {
    ...
    data.forEach((l: any) => {
      ...
      if (!email || suppressed.has(email) || alreadySent.has(email) || seen.has(email)) return;
  ...
  console.log(`📋 Total clean fresh unsent leads available: ${freshContacts.length}`);
  ...
  // Combine and slice exactly 497 leads
  const selectedLeads = freshContacts.slice(0, 497);
  console.log(`🎯 Selected exactly ${selectedLeads.length} leads for this campaign blast.`);

  // Group the 497 leads by niche
  ...
  if (DRY_RUN) {
    ...
    for (const niche of Object.keys(pools)) {
      ...
      console.log(`  Sample: ${pools[niche][0].email} | Name: ${pools[niche][0].firstName} | Company: ${pools[niche][0].company}`);
    }
    console.log('\n✅ Dry run complete. Run without --dry-run to send campaigns.');
    return;
  }

  // LIVE execution in Brevo
  ...
  for (const niche of Object.keys(pools)) {
    ...
    const listName = `Jun22-Blast-${niche}`;
    const tmpl = TEMPLATES[niche];
    ...
    console.log(`  ✅ Campaign scheduled for immediate send (ID: ${campRes.id})`);

    leads.forEach(l => sentLog.push(l.email));
  }

  // Save sent log
  const logPath = path.join(process.cwd(), 'scripts/sent_june22_blast_497.json');
  fs.writeFileSync(logPath, JSON.stringify(sentLog, null, 2));
  console.log(`\n💾 Sent log saved: ${logPath} (${sentLog.length} emails)`);
  console.log(`\n✅ Done! ${grandTotal} emails scheduled to send immediately.`);
}
