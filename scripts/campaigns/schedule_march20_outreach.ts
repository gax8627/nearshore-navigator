/**
 * March 20th Fresh Outreach — 500 New Leads
 *
 * Friday Campaign — Targeting next segment of manufacturing leads.
...
const SENT_LOG_PATH = path.join(process.cwd(), 'scripts/sent_march20_outreach.json');

const DRY_RUN = process.argv.includes('--dry-run');
const BATCH_LIMIT = 500;

// Schedule for TODAY 10:15 AM MT (17:15 UTC)
const SCHEDULED_AT = '2026-03-20T17:15:00.000Z';
...
const PREVIOUS_SENT_LOGS = [
  ...
  path.join(process.cwd(), 'scripts/sent_march18_outreach.json'),
];

async function main() {
  console.log('\n🚀 March 20th Fresh Outreach — 500 New Leads (Friday 10:15 AM MT)');
  ...
  if (!DRY_RUN) {
    fs.writeFileSync(SENT_LOG_PATH, JSON.stringify(existingSent, null, 2));
    console.log(`\n✅ Done! ${sentCount} emails queued for Mar 20 @ 10:15AM MT.`);
