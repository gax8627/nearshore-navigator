/**
 * March 5th Scheduled Outreach - 250 Leads
 * 
 * Run preview:  npx tsx scripts/campaigns/schedule_march5_outreach.ts --dry-run
 * Run for real: npx tsx scripts/campaigns/schedule_march5_outreach.ts
 */
...
const SENT_LOG_PATH = path.join(process.cwd(), 'scripts/sent_march5_outreach.json');
...
async function main() {
  console.log('\\n🚀 Scheduled 250 Manufacturing Outreach Campaign (Mar 5, 9 AM PST)');
  ...
  for (const logPath of PREVIOUS_SENT_LOGS) {
      if (fs.existsSync(logPath)) {
          try {
              ...
              // Handle flat array of emails or object arrays with 'email' properties
              if (Array.isArray(data)) {
                  data.forEach(item => {
                      if (typeof item === 'string') {
                           sentSet.add(item.toLowerCase().trim());
                      } else if (item.email) {
                           sentSet.add(item.email.toLowerCase().trim());
                      }
                  });
              } else if (typeof data === 'object') {
                  // Engagement map cases
  ...
  const TARGET_FILES = [
    ...
    // New additions to find more fresh leads
  ...
  for (let i = 0; i < leads.length; i++) {
    ...
    if (DRY_RUN) {
      if (i < 3) console.log(`[DRY RUN] Would schedule to: ${lead.email} | ${lead.company}`);
      sentCount++;
    } else {
      try {
        await brevo.sendEmail({
          ...
          subject,
          htmlContent: html,
          scheduledAt: SCHEDULED_AT // The essential param for scheduling
        });
  ...
  if (!DRY_RUN) {
    fs.writeFileSync(SENT_LOG_PATH, JSON.stringify(existingSent, null, 2));
    console.log(`\\n✅ Scheduling complete! ${sentCount} new emails queued for Mar 5 @ 9AM PT.`);
  } else {
    console.log(`\\n✅ Dry run complete. ${sentCount} new emails queued for logic verification.`);
  }
}
