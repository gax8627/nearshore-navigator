...
async function main() {
    ...
    const email = 'gax8627@gmail.com';
    const name = 'User';
    const company = 'Nearshore Navigator';

    console.log(`🧪 Sending test Zoom Outreach email to ${email}...`);

    const htmlContent = wrapHtml(`
        <p>Hi ${name},</p>
        <p>Following up on your interest in the <strong>USMCA Audit protocols</strong>.</p>
        <p>The upcoming "100% Rules of Origin" mandate is more than a compliance hurdle—it's an operational shift that will define the competitive landscape in Aerospace, Defense, and Medical sectors for the next decade.</p>
        <p>I'd like to offer you a 15-minute <strong>Strategic Exposure Audit</strong> over Zoom. We’ll look at your current supply chain map and identify exactly where the new July thresholds create risk—or opportunity—for <strong>${company}</strong>.</p>
    ...
    try {
        await brevo.sendEmail({
            to: [{ email, name }],
            subject: `TEST: ${company} — Strategic USMCA Exposure Audit?`,
