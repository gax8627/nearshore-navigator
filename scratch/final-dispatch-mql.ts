...
async function finalDispatch() {
    ...
    try {
        console.log('🚀 MISSION: Full MQL Outreach - Meeting Focus...');

        // 1. Create a fresh clean list (List 17)
        ...
        console.log(`✅ List 17 Created (ID: ${newListId})`);
        ...
        console.log(`📥 Syncing ${mqlLeads.length} MQLs to List ${newListId}...`);

        // Use Bulk Import for speed and reliability
        const importRes = await fetch('https://api.brevo.com/v3/contacts/import', {
            ...
            body: JSON.stringify({
                contactsToImport: mqlLeads.map((l: any) => ({
                    email: l.email,
                    attributes: {
        ...
        // 3. Wait a few seconds for Brevo to process the import
        console.log('⏳ Waiting for import processing...');
        await new Promise(resolve => setTimeout(resolve, 5000));

        // 4. Create the Meeting-Focus Campaign
        const htmlContent = `
            <div style="background-color: #030712; color: #f3f4f6; padding: 40px; font-family: sans-serif; line-height: 1.6;">
                <div style="max-width: 600px; margin: 0 auto;">
                    ...
                    <p style="margin-top: 40px; border-top: 1px solid #374151; pt-20px; font-size: 0.9em; color: #9ca3af;">
                        Best regards,<br/>
        ...
        if (campData.id) {
            ...
            console.log('🏁 MISSION COMPLETE: Final MQL outreach is live.');
        } else {
